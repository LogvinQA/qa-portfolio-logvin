...
HTML-отчёт сгенерирован автоматически после запуска Postman-коллекции через Newman.  
Отчёт показывает общее количество тестов, статус-коды и время отклика.

![Newman Report Screenshot](./screenshots/newman_report_example.png)

✅ Все тесты завершились успешно — статус **200 OK**, формат ответа **JSON**.
